...
const ENGAGEMENT_CARDS = [
  ...
  },
  {
    icon: Award,
    dept: '副指導教員制度',
    title: '健康栄養学科の専門教員のゼミで、スポーツを学ぶ',
    body: '食物栄養学科に在籍したまま、副指導教員制度を活用することで、健康栄養学科のスポーツ専門教員のゼミ活動に参加できます。公認スポーツ栄養士や運動生理学の専門家から直接指導を受ける機会が、在学中に広がります。',
...
const FACULTY = [
  {
    ...
    message:
      '競技者を対象とした食事調査を中心に、栄養アセスメント研究の第一人者として国内外の学会で多数の発表を重ねる。本学大学院（食物栄養学専攻修士課程）の研究指導も担当しており、学部で学んだ学生がさらに専門性を高めるために教授ゼミへ進学する道も開かれている。実際に、大学院生（食物栄養学科卒業生）が競技者の食事調査に関する研究で、昨年の日本スポーツ栄養学会大会において演題賞を受賞——「入学・学部・大学院・キャリアアップ」という一貫した学びの好例を、自らのゼミから生み出している。',
  },
  {
    name: 'ヨーコ ゼッターランド 講師',
    dept: '健康栄養学科',
...
const HEALTH_SPORT_TEACHERS = [
  {
    name: '徳野 裕子 准教授',
    courses: '公衆栄養学（演習・実習）',
    desc: '現役の公認スポーツ栄養士として、実際にスポーツ選手への栄養サポート活動を重ねてきた実践家。健康栄養学科の必修科目を担当しており、副指導教員制度を通じて指導を受けることができる。',
  },
  {
    name: '飯田 路佳',
  ...
  {
    name: '若葉 京良',
    courses: '健康・運動・栄養の科学、スポーツ実技・指導法Ａ、運動生理学（生理学を含む）',
    desc: '運動生理学を中心に、解剖学・生理学の基礎から競技パフォーマンス向上まで体系的に指導。健康運動指導士資格取得に必須の科目を担当。',
  },
  {
    name: '和田 左千子',
  ...
  },
  {
    ...
    desc: 'オリンピック・プロリーグ・放映権・地域スポーツ振興など、スポーツの社会的価値とビジネス構造を多角的に解説。食物栄養・健康栄養をはじめ全学科の学生が受講可能。',
  },
];
...
const ROUTES = [
  ...
  },
  {
    ...
    title: 'スポーツ栄養の専門家として',
    steps: [
      { label: '健康栄養学科に入学', note: 'スポーツ栄養を軸に栄養士・管理栄養士を目指す' },
      { label: '必修科目でスポーツ栄養の第一線を学ぶ', note: '公認スポーツ栄養士の資格を持つ教員から直接指導を受ける' },
      { label: '栄養士資格取得（卒業時）', note: '教員免許・健康運動指導士など、卒業時に取得できる資格も並行して取得可能。スポーツ栄養関連のキャリアへの入口となる' },
      { label: '実務経験を積みながら管理栄養士資格取得へ', note: '栄養士として就業後、実務経験を経て管理栄養士国家試験の受験資格を得る' },
      { label: 'スポーツ栄養関連のキャリアへ', note: '現場でスポーツと栄養をつなぐ専門家として活躍。栄養サポート経験を一歩ずつ積み重ねていく' },
      { label: '公認スポーツ栄養士取得（任意）', note: '管理栄養士免許取得後、十分な実務・栄養サポート経験を重ねてから受験可能', optional: true },
    ],
  },
  {
    color: 'var(--stone)',
...
const COURSES = [
  ...
  },
  {
    ...
    meta: ['前期', '2単位', '講義・演習'],
    overview: '健康栄養学科「コーチング論」・食物栄養学科「スポーツコーチング論」として2学科合同で開講される実践的科目。人を育てる「コーチング」の本質を学ぶ。「ティーチング」との違いを起点に、「グッドコーチ」に必要な資質と考え方を探究する。体罰・ハラスメントのない指導環境、「プレーヤーズセンタード」「アントラージュ」「Well Being」の概念と実践、日米のスポーツ文化の違いを学ぶ。ロールプレイングと学生間ディスカッションを通じて指導者としての実践力を養う。',
    topics: [
      'コーチングとティーチングの違い',
      '成長過程と指導方法の選択',
      '効果的なコーチングの言葉',
        ...
        <div className="lab-page__content">
          ...
          >
            ...
            <p className="lab-story-feature__lead">
              ...
              さらに<strong>副指導教員制度</strong>を活用すれば、健康栄養学科のスポーツ専門教員のゼミ活動にも参加できます。公認スポーツ栄養士の資格を持つ教員から直接指導を受ける機会が、食物栄養学科に在籍したまま開かれています。<br /><br />
          ...
          <motion.section className="lab-section" {...fadeUp(0.05)}>
            <h2 className="lab-section__title">十文字学園女子大学でのスポーツ栄養との関わり方</h2>
            <p className="lab-section__body">
              食物栄養学科の学生が、スポーツと栄養の接点を持てる3つの切り口があります。
            </p>
          ...
          <motion.section className="lab-section" {...fadeUp(0.05)}>
            ...
            <p className="lab-section__body">
              スポーツ栄養学・運動生理学は食物栄養学科の選択科目として設置。コーチング論（スポーツコーチング論）は健康栄養・食物栄養の<strong>2学科合同で開講</strong>されており、どちらの学科の学生も一緒に学びます。担当教員のご紹介はこのページの「講義・教員紹介」をご覧ください。
            </p>
          ...
          {/* ===== 講義・教員紹介 ===== */}
          <motion.section className="lab-section" {...fadeUp(0.05)}>
            <h2 className="lab-section__title">健康栄養学科のスポーツ系教員</h2>
            <p className="lab-section__body">
              以下は健康栄養学科に所属するスポーツ関連の教員です。「コーチング論／スポーツコーチング論」は食物栄養・健康栄養の<strong>2学科同時開講</strong>のため、食物栄養学科の学生もそのまま受講できます。それ以外の科目・ゼミについても、<strong>副指導教員制度</strong>を活用することで、食物栄養学科在籍のまま指導を受けることができます。
            </p>
